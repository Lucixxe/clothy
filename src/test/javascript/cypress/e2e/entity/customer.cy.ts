...
describe('Customer e2e test', () => {
  ...
  const customerSample = {
    email: 'Berthe45@yahoo.fr',
    firstName: 'Sophie',
    name: 'trop peu au-dedans de communauté étudiante',
    createdAt: '2025-09-07T18:44:17.040Z',
    passwordHash: 'parce que de sorte que',
    address: 'chut soudain à la faveur de',
  };
  ...
  describe('new Customer page', () => {
    ...
    it('should create an instance of Customer', () => {
      cy.get(`[data-cy="email"]`).type('Adrien.Moulin4@gmail.com');
      cy.get(`[data-cy="email"]`).should('have.value', 'Adrien.Moulin4@gmail.com');

      cy.get(`[data-cy="firstName"]`).type('Guillemette');
      cy.get(`[data-cy="firstName"]`).should('have.value', 'Guillemette');

      cy.get(`[data-cy="name"]`).type('par rapport à');
      cy.get(`[data-cy="name"]`).should('have.value', 'par rapport à');

      cy.get(`[data-cy="createdAt"]`).type('2025-09-07T16:29');
      cy.get(`[data-cy="createdAt"]`).blur();
      cy.get(`[data-cy="createdAt"]`).should('have.value', '2025-09-07T16:29');

      cy.get(`[data-cy="passwordHash"]`).type('camarade');
      cy.get(`[data-cy="passwordHash"]`).should('have.value', 'camarade');

      cy.get(`[data-cy="address"]`).type('crac');
      cy.get(`[data-cy="address"]`).should('have.value', 'crac');
