...
import static com.clothy.myapp.domain.CategoryAsserts.*;
...
class CategoryResourceIT {
    ...
    public static Category createEntity() {
        return new Category().name(DEFAULT_NAME).slug(DEFAULT_SLUG).isActive(DEFAULT_IS_ACTIVE);
    }
    ...
    public static Category createUpdatedEntity() {
        return new Category().name(UPDATED_NAME).slug(UPDATED_SLUG).isActive(UPDATED_IS_ACTIVE);
    }
    ...
    void putExistingCategory() throws Exception {
        ...
        updatedCategory.name(UPDATED_NAME).slug(UPDATED_SLUG).isActive(UPDATED_IS_ACTIVE);
