...
public interface OrderItemRepository extends JpaRepository<OrderItem, Long> {
    ...
    @Query(
        value = "select orderItem from OrderItem orderItem left join fetch orderItem.order left join fetch orderItem.product",
        countQuery = "select count(orderItem) from OrderItem orderItem"
    )
    Page<OrderItem> findAllWithToOneRelationships(Pageable pageable);

    @Query("select orderItem from OrderItem orderItem left join fetch orderItem.order left join fetch orderItem.product")
    List<OrderItem> findAllWithToOneRelationships();

    @Query(
        "select orderItem from OrderItem orderItem left join fetch orderItem.order left join fetch orderItem.product where orderItem.id =:id"
    )
