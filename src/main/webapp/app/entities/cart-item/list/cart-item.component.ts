...
import { CartItemService, EntityArrayResponseType } from '../service/cart-item.service';
...
export class CartItemComponent implements OnInit {
  ...
  trackId = (item: ICartItem): number => this.cartItemService.getCartItemIdentifier(item);
      ...
      .pipe(
        ...
        tap(() => {
          if (this.cartItems().length === 0) {
            ...
            this.cartItems.set(this.refineData(this.cartItems()));
          }
        }),
      )
      .subscribe();
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.cartItems.set(this.refineData(dataFromBody));
  }
