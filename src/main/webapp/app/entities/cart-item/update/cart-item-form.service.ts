...
type CartItemFormDefaults = Pick<NewCartItem, 'id' | 'isInOrder'>;

type CartItemFormGroupContent = {
  id: FormControl<ICartItem['id'] | NewCartItem['id']>;
  quantity: FormControl<ICartItem['quantity']>;
  unitPrice: FormControl<ICartItem['unitPrice']>;
  lineTotal: FormControl<ICartItem['lineTotal']>;
  isInOrder: FormControl<ICartItem['isInOrder']>;
  cart: FormControl<ICartItem['cart']>;
  product: FormControl<ICartItem['product']>;
  customerOrder: FormControl<ICartItem['customerOrder']>;
};
...
export class CartItemFormService {
  createCartItemFormGroup(cartItem: CartItemFormGroupInput = { id: null }): CartItemFormGroup {
    ...
    return new FormGroup<CartItemFormGroupContent>({
      ...
      unitPrice: new FormControl(cartItemRawValue.unitPrice, {
        validators: [Validators.required],
      }),
      lineTotal: new FormControl(cartItemRawValue.lineTotal, {
        validators: [Validators.required],
      }),
      isInOrder: new FormControl(cartItemRawValue.isInOrder, {
        validators: [Validators.required],
      }),
      cart: new FormControl(cartItemRawValue.cart),
      product: new FormControl(cartItemRawValue.product),
      customerOrder: new FormControl(cartItemRawValue.customerOrder),
    });
  }
