...
import { CartService, EntityArrayResponseType } from '../service/cart.service';
...
export class CartComponent implements OnInit {
  ...
  trackId = (item: ICart): number => this.cartService.getCartIdentifier(item);
      ...
      .pipe(
        ...
        tap(() => {
          if (this.carts().length === 0) {
            ...
            this.carts.set(this.refineData(this.carts()));
          }
        }),
      )
      .subscribe();
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.carts.set(this.refineData(dataFromBody));
  }

  protected refineData(data: ICart[]): ICart[] {
